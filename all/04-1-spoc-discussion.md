...
1. 寄存器、高速缓存、内存、外存的访问特征？
   * 访问时间依次增加
   * 访问频率依次减少
   * 寄存器位于CPU内部，cache位于CPU内部或者外部，内存和外存位于CPU外部
   * 高速换粗、内存、外存的管理均需要软件来辅助
2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？
   * 各种存储介质都在工作，但是没有哪个介质称为系统运行的瓶颈，任务合理地分配到不同的存储介质中
3. 在你写程序时遇到过内存不够的情况吗？尝试过什么解决方法？
   * 在一些空间复杂度高的算法中可能会有内存不足的情况
   * 一般可以改进算法，使用就地算法节省空间
   * 动态链接库
...
1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？
   * 覆盖：给程序划分不同的功能块，不会同时执行的模块可以共享同一块内存
   * 任务：模块的划分，确定模块对内存的共享关系
2. 什么是交换技术？覆盖与交换有什么不同？
   * 交换：进程所占用的内存空间作为整体从内存换入换出
   * 覆盖通过进程切块解决了进程所需要内存不够的情况。而交换通过进程的换入换出，解决了多个进程导致物理内存不够的情况，保证了高优先级的进程正常运行
3. 覆盖和交换技术在现代计算机系统中还有需要吗？可能用在什么地方？
   * 可能会使用
   * 在某些专用的计算机上，或者对内存需求非常大程序中可能会使用
4. 如何分析内核模块间的依赖关系？
   * 使用depmod命令
5. 如何获取内核模块间的函数调用列表？
   * 静态代码分析，动态函数调用跟踪
...
1. 什么是时间局部性、空间局部性和分支局部性？
   * 如果存储器的某个位置被访问了则在短时间内它再次被访问的概率很高
   * 如果存储器的某个位置被访问了则在短时间内其附近的位置被访问的概率很高
   * 某个分支语句在一段时间内分支的选择情况很可能相同
2. 如何提高程序执行时的局部性特征？
   * 使用缓存可以利用时间和空间局部性
   * 进行分支预测
...
1. 什么是虚拟存储？它与覆盖和交换的区别是什么？它有什么好处和挑战？
   * 将当前执行程序所需要的部分内容放到内存中，将剩下的放入外存中
   * 区别：不连续，部分交换，不需要程序员手动划分程序，结合了覆盖和交换的特点
   * 好处：有很强的虚拟性，让用户可使用很大的逻辑空间，只进行部分交换，内存管理非常灵活
   * 挑战：内存管理更加复杂
2. 虚拟存储需要什么样的支持技术？
   * 硬件：地址转换技术
   * 软件：置换算法
...
### 8.5 虚拟页式存储
 ...
 1. 什么是虚拟页式存储？缺页中断处理的功能是什么？
    * 不将程序的所有页面都加载进去，而是把硬盘当作虚拟内存，在需要时候动态进行页面的换入换出
    * 缺页中断可以帮助加载不在内存中的页面
...
3. 页式存储和虚拟页式存储的区别是什么？
   * 虚拟页式存储是建立在页式存储机制上的，虚拟页式存储可以充分利用外存，将暂时不用的页存放到外存中，动态地换入换出
