...
* 特权指令只能在特权级0使用，在不同的特权级下能够访问的数据范围不同，同样一条指令在不同的特权级下的行为可能会有所不同


3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。

* 主要作用是保护内存
* 通过门访问时，低优先级代码可以访问高优先级服务
* 访问段时，高优先级代码访问优先级低的数据
...
* 访问数据时，需要检查当前特权级是否是高于需要访问的数据段的DPL
* 如果通过段寄存器SS访问栈数据时，需要保证CPL = DPL， RPL = DPL
* 如果代码涉及到控制转移时一般只允许低特权级调用高特权级代码
* 发生中断或者异常时，需要判断当前的门描述符的特权级和目标代码的特权级
* 在不使用调用门的情况下访问非一致代码段：需要保证CPL =DPL
* 使用调用门时DPL规定了当前执行程序可以访问该调用门的最低特权级
* 访问一致代码段以及通过调用门访问非一致代码段：DPL规定了访问最高特权级，需要保证DPL <= CPL
...
* 压栈的内容不同，从用户态转换到内核态时，多向栈里压入了SS和ESP的值


4. 如何利用int和iret指令完成不同特权级的切换？

* 需要手工进行所需要的栈结构的构造，之后调用指令


5. TSS和Task Register的作用是什么？

* TSS用于操作系统进程管理过程中，保存进程切换时的任务现场信息。

* TR保存16位段选择子，32位基地址，16位段界限和当前任务的TSS属性，基地址表明TSS的第一个字节的线形地址，段界限确定TSS的字节个数。
...
* 不考虑多级页表等情况，一般转换过程为：逻辑地址 -> 线形地址 -> 物理地址

2. 描述X86-32的MMU地址转换过程；

先通过虚页号查页表，找到物理页号，之后将物理页号和页内位移拼接在一起即得到了相应的物理地址，此时查表的过程中可能会产生页缺失等问题，如果发生页缺失则将相应的页从外存导入到内存中。

### 7.4 了解UCORE建立段/页表

1. 分析MMU的使能过程，尽可能详细地分析在执行进入保护械的代码“movl %eax, %cr0 ; ljmp $CODE_SEL, $0x0”时，CPU的状态和寄存器内容的变化。

* 预先构建好全局描述符表，使用lgdt命令将其load进入内存
* 设置在CR0寄存器中将PE flag 置1
* 使用ljmp更新CS寄存器的值(ljmp实际上跳转到了下一条指令)，进入保护模式
